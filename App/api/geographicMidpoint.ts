...
function radToCart(radianLocations: Array<GeographicLocation>) {
	...
	radianLocations.forEach(function(radianLocation: GeographicLocation) {
		const cartesianLat = Math.cos(radianLocation.latitude) * Math.cos(radianLocation.longitude);
		const cartesianLng = Math.cos(radianLocation.latitude) * Math.sin(radianLocation.longitude);

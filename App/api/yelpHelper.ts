...
import {
  getBusinessesByCoordinatesAndCategories,
  Business,
} from './yelpApi';
...
export async function generateOrderedRestaurantList(
  locations: Array<{ latitude: number; longitude: number }>,
  categoryAliasLists: Array<Array<string>>,
  pricePreferences: Array<number>,
  limit: number | undefined = undefined
): Promise<Restaurant[]> {
  const midpoint = geoMidpoint(locations);

  // concatenate all preference lists into one list
  const preferenceList: Array<string> = categoryAliasLists.reduce(
    (acc, val) => acc.concat(val),
    []
  );
  // remove duplicates and order by element frequency
  const orderedPreferenceList: Array<string> = preferenceList
    .filter((item, pos) => preferenceList.indexOf(item) == pos)
    .sort(
      (a, b) =>
        preferenceList.filter((v) => v === a).length -
        preferenceList.filter((v) => v === b).length
    )
    .reverse();
  ...
  pricePreferencesString =
    pricePreferencesString.length > 0 ? pricePreferencesString : '1,2,3,4';

  // get businesses from Yelp API
  const businesses: Business[] = await getBusinessesByCoordinatesAndCategories(
    midpoint.latitude,
    midpoint.longitude,
    orderedPreferenceString,
    pricePreferencesString,
    limit
  );

  // convert businesses to Restaurant objects
  const restaurants: Restaurant[] = businesses.map((business) =>
    businessToRestaurant(business)
  );
...
export async function generateExploreSections(): Promise<
  Array<{ sectionTitle: string; contentData: any }>
> {
  ...
  let prefs: string[] = await axios
    .get(SERVER_URI + '/users/' + profileId + '/prefs')
    .then((response) => response.data.prefs);
  ...
  const location = { latitude: 29.648292, longitude: -82.345171 };
  const { latitude, longitude } = location;

  // get restaurants for each pref
  const restaurants: Restaurant[][] = await Promise.all(
    prefs.map(async (pref) => {
      const restaurants: Restaurant[] = await generateOrderedRestaurantList(
        [{ latitude, longitude }],
        [[pref]],
        [1, 2, 3, 4],
        10
  ...
  const stuff = prefs.map((pref, index) => {
    return {
      sectionTitle: pref,
